...
const char* FTPssid = "homeinternet"; //for now this is how we are going to connect using the FTP server
const char* FTPpassword = "trixie4me"; //I want to change this to make the code more lean.
...
display.setFont(ArialMT_Plain_10);
 display.drawString(5, 0, "Tandy 102 WiFi SD card");
 
display.drawString(16,9,"LeRoy Miller, 2019");
display.drawString(0,19,"Based on: MyTeSed By");
display.drawString(0,29,"David Stephenson, 2017");
display.drawString(8,39,"C64-WiFiModem by");
display.drawString(0,49,"my2bits & others, 2017");
display.display();
delay(8000);
display.clear();
display.drawString(5, 0, "Tandy 102 WiFi SD card");
display.drawString(0,10, "FTP Server by nailbuster");
display.drawString(10,20, "& robo8080, 2017");
display.drawString(17,35, "Menu Code by");
display.drawString(5,45, "LeRoy Miller, 2019");
display.display();
delay(8000);
display.clear();
...
while(digitalRead(BUTTON) != LOW) {
  ...
  if (digitalRead(UP) == LOW) {
    if (menuLevel < maxMenuLevel) { menuLevel++; }
    else {menuLevel = maxMenuLevel;}
  }
...
display.clear();
display.setFont(ArialMT_Plain_10);
display.drawString(0,0,"Restarting in 5 seconds.");
display.display();
delay(5000);
ESP.restart();
}
...
void drawMenu() {
  //display.clear();
  display.drawString(5, 0, "Tandy 102 WiFi SD card");
  display.drawString(35,10,"Main Menu!");
  display.drawString(12, 24, "SD Card Storage");
  display.drawString(12, 34, "WiFi Modem");
  display.drawString(12, 44, "FTP Server");
